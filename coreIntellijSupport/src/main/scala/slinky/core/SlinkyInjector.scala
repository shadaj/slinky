...
class SlinkyInjector extends SyntheticMembersInjector {
  ...
  def createComponentBody(cls: ScTypeDefinition): Seq[(String, InjectType)] = {
    ...
    val stateDefinition: Option[(String, InjectType)] =
      types.forName("State").iterator.toSeq.headOption.flatMap { elm =>
        elm.namedElement match {
          case alias: ScTypeAliasDefinition =>
    ...
    val snapshotDefinition: Option[(String, InjectType)] =
      types.forName("Snapshot").iterator.toSeq.headOption.flatMap { elm =>
        elm.namedElement match {
          case alias: ScTypeAliasDefinition =>
            Some((alias.getText, Member))
          case propsCls: ScClass if propsCls.isCase =>
  ...
  def createExternalBody(cls: ScTypeDefinition): Seq[(String, InjectType)] = {
    val applyMethods = cls.extendsBlock.members.collect { case td: ScTypeDefinition =>
      td
    }.find(_.name == "Props")
      .map {
        case propsCls: ScClass if propsCls.isCase =>
