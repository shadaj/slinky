...
@react object DocsPage {
  def docsFilePath(props: js.Dynamic) = {
    val matchString = props.selectDynamic("id").toString//props.selectDynamic("match").params.selectDynamic("0").toString
  ...
  val component = FunctionalComponent[js.Dynamic] { props =>
    ...
    "hi"
    // val matchString = props.selectDynamic("id").toString//props.selectDynamic("match").params.selectDynamic("0").toString
    // val selectedGroup = DocsTree.tree.find(_._2.exists(_._2 == matchString)).get._1
  ...
  //   @JSExportTopLevel(name = "default", moduleID = "[...id]")
