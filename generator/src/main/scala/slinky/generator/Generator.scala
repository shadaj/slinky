    ...
    allSymbols.foreach { case (symbol, (tags, attrs)) =>
      ...
      val tagsGen = tags.map { t =>
        s"""type tagType = tag.type
           |override def apply(mods: TagMod[tag.type]*): WithAttrs = macro slinky.core.TagMacros.tagApply"""
      }
      ...
      val attrsGen = attrs.toList.flatMap { a =>
        ...
        val noEvent = s"""@inline @slinky.core.createAttrMethod("${a.attributeName}") def :=(v: () => Unit) = new AttrPair[_${symbolWithoutEscape}_attr.type]("${a.attributeName}", v)""".stripMargin
      ...
      val symbolExtendsList = (if (tags.nonEmpty) Seq(s"""Tag("${tags.head.tagName}")""") else Seq.empty) ++ (if (attrs.isDefined) Seq("Attr") else Seq.empty)

      val annotations = if (tags.nonEmpty) s"""@slinky.core.tagObject("${tags.head.tagName}")""" else ""
      ...
      out.println(
        s"""package $pkg
           ...
           | */
           |$annotations object $symbol $symbolExtends {
