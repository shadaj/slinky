...
object MDN extends TagsProvider {
  ...
  val supportedAttributes: Set[String] =
    """accept acceptCharset accessKey action allowFullScreen allowTransparency alt
      ...
      |style summary tabIndex target title type useMap value width wmode wrap""".stripMargin
      .split('\n')
      .flatMap(_.split(' '))
      .toSet
  ...
  lazy val globalAttributes: List[(Attr, String)] = {
    ...
    extraAttributes ::: attributesSection
      .grouped(2)
      .flatMap { i =>
        val attr = i.head
        val docs = i.last
      ...
      val attributes = attributesSection.flatMap { dl =>
        ...
        val attrsAndDocs = children.foldLeft(Seq.empty[(String, String)]) { (acc, cur) =>
          if (cur.tagName == "dt") {
            ...
            acc.init :+ acc.last.copy(_2 =
              if (acc.last._2.isEmpty) cur.innerHtml else acc.last._2 + " " + cur.innerHtml
            )
          }
        }
  ...
  val tags: Seq[String] =
    """a abbr address area article aside audio b base bdi bdo big blockquote body br
      ...
      |thead time title tr track u ul var video wbr""".stripMargin.split('\n').flatMap(_.split(' ')).toSeq
  ...
  def extract: (Seq[Tag], Seq[Attribute]) = {
    ...
    val attrs = tagsWithAttributes
      .flatMap(v => v._2.map(t => (v._1, t._1, t._2)))
      .groupBy(_._2)
      .map { case (attr, instances) =>
