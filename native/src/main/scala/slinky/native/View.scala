...
case class NativeTouchEvent(changedTouches: Seq[Any],
                            identifier: String,
                            locationX: Int,
                            locationY: Int,
                            pageX: Int,
...
case class LayoutRectangle(x: Int, y: Int,
                           width: Int, height: Int)

case class LayoutChangeEvent(layout: LayoutRectangle)

@react object View extends ExternalComponent {
  case class Props(onStartShouldSetResponder: js.UndefOr[NativeSyntheticEvent[NativeTouchEvent] => Unit] = js.undefined,
