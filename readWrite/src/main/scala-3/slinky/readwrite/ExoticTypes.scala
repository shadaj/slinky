...
// Extra information about types that can't be obtained using deriving API
object ExoticTypes {
  ...
  object NominalUnion {
    ...
    transparent inline implicit def get[T]: NominalUnion[T] = ${impl[T]}

    def impl[T: Type](using q: Quotes): Expr[NominalUnion[T]] = {
      import q.reflect._
  ...
  // Scala wouldn't provide any mirror for non-case value class at all.
  trait ValueClass[T] {
    type Repr
    def from(t: T): Repr
    def to(r: Repr): T
  ...
  object ValueClass {
    ...
    transparent inline implicit def get[T]: ValueClass[T] = ${impl[T]}

    def impl[T: Type](using Quotes): Expr[ValueClass[T]] = {
      val q = summon[Quotes]
      import q.reflect._
      val T = TypeRepr.of[T]
  ...
  object DefaultConstructorParameters {
    /*transparent*/ inline implicit def get[T]: DefaultConstructorParameters[T] =
      ${impl[T]}

    def impl[T: Type](using q: Quotes): Expr[DefaultConstructorParameters[T]] = {
      import q.reflect._
